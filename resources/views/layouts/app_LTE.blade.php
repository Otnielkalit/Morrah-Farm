        ...
        <nav class="main-header navbar navbar-expand navbar-white navbar-light">
            ...
            <ul class="navbar-nav ml-auto">
                ...
                @php
                    ...
                    $stocks = \DB::select(
                        'SELECT s.jumlah, p.* FROM stok AS s JOIN produks AS p ON p.stok_id = s.id WHERE s.jumlah <= 5',
                    );
                    $orders = \DB::select('SELECT * from pesanans where status = 2');
                    $produkKadaluwarsa = \DB::select(
                        'SELECT p.nama_produk, s.id, s.kadaluwarsa, s.jumlah FROM produks AS p JOIN stok s ON p.stok_id = s.id WHERE s.kadaluwarsa < DATE_ADD(CURDATE(), INTERVAL 3 DAY)',
                    );
                ...
                <li class="dropdown dropdown-list-toggle"><a href="#" data-toggle="dropdown"
                        ...
                        <span
                            class="badge headerBadge1 bg-danger">{{ count($stocks) + count($orders) + count($produkKadaluwarsa) + count($produkpackings) }}</span>
                    </a>
